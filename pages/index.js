...
import { useMetaMaskAccount } from "../components/metaMask-accountProvider";

export default function Home() {
  const { ethereum, connectedAccount, connectAccount} = useMetaMaskAccount();

  //state variables for ethereum window and accounts
  const [keyboards, setKeyboards] = useState([])
  //state variable for keyboards from contract
  ...
  const addContractEventHandlers = () => {
    //notification if new keyboard is created
    if (keyboardsContract && connectedAccount) {
      keyboardsContract.on('KeyboardCreated', async (keyboard) => {
        ...
        await getKeyboards();
      })//notification if user is tipped
